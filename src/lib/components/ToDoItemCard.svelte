...
<div class="flex-auto p-4 shadow-lg" bind:this={nodeRef}>
    ...
    <ul on:click={routeToDetails} class="pl-4">
        <li>Created: <span class="text-lime-600 pr-4">{new Date(data.createdAt).toDateString()}</span></li>
        {#if data.deadline}
        <li>Deadline: <span class="text-red-600">{new Date(data.deadline).toDateString()}</span></li>
        {/if}
    </ul>
    <div class="text-right">
        <button on:click={handleDelete} class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-2 rounded inline-flex items-center">
            <svg class="fill-current w-4 h-4 text-red-800" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 330 330" style="enable-background:new 0 0 330 330;" xml:space="preserve">
                <g id="XMLID_6_">
                    <g id="XMLID_11_">
